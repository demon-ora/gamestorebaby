...
if (isset($_POST['btnsave'])) {
 ...
 // validate image
  ...
  if (count($err) == 0) {
    ...
    $sql = "INSERT INTO game(title,catagories,type,image,url,descripation) VALUES ('$title','$cate','$gametype','$ok','$loc','$descripation')";
    //execute query
    if ($connection->query($sql)){  
    header("location:dashboardgamee.php");
    } else {
...
<?php foreach($data as $key => $value){ ?>
  <tr>
    ...
    <td> <img src="<?php echo $value['image'] ?>" height="50" width="50" style="object-fit: cover;">
     </td>
    <td><?php echo $value['url'] ?></td>
    <td><?php echo $value['descripation'] ?></td>
    <td>
      <a href="deletegame.php?id=<?php echo $value['id'] ?>" onclick="return confirm('Are you sure to delete?')"><i class="fas fa-trash-alt fa-2x"></i></a></td>
     <td> <a href="editgame.php?id=<?php echo $value['id'] ?>">
